...
Page({
    ...
    onLoad: function (options) {
        ...
        core.requestApi(app.globalData.shopApiUrl, {
            ...
            var customer_num_left = res.data.goods_info.team_num - filtTeam.length;
            that.setData({
                ...
                customer_num_left: customer_num_left,
                users_num: users.length
            });
            if(customer_num_left == res.data.goods_info.team_num){

            }
            if(customer_num_left==0){
                that.setData({'opBtnText': '此团已成，运费会充到账户余额'});
                that.setData({'openType': ''});
            }else{
                var s_miao = res.data.s_miao;
      ...
      var that = this;
        return {
          title: '快来' + that.data.goods_info.team_price 
            ...
            path: '/pages/share_tuan2/share_tuan?team_sign=' + team_sign
        }
    },
